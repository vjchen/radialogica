    ...
    if len(word) < 2:
        ...
        for i, character in enumerate(word):
            if not character in word[:i]: #make sure no duplication
                for j in create_anagrams(word[:i]+word[i+1:]): #recursively swap outer characters to create anagrams
                    yield j+character #use generator to save memory when word gets long.
